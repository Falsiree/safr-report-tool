import type { InjuryOption } from './models/injury-options'

export const StandardComplaints: InjuryOption[] = [{
    injuryName: 'Animal Attack',
    subInjuries: null,
    treatments: [{
      label: 'Oxygen Mask',
      reportText: 'Oxygen mask applied to patient.',
      emote: ''
    },{
      label: 'Wounds packed',
      reportText: 'patients wounds were packed with gauze to stop bleeding.',
      emote: ''
    },{
      label: 'IV Saline and TXA',
      reportText: 'IV saline and TXA administered.',
      emote: ''
    },{
      label: 'Flush/Pack/Wrap',
      reportText: 'Wounds flushed with IV saline, packed with fresh gauze, and wrapped in bandages.',
      emote: ''
    },{
      label: 'C-Collar',
      reportText: 'Patient was made stable with a C-collar and spine board.',
      emote: ''
    }]
  }, {
    injuryName: 'Animal Attack (Minor)',
    subInjuries: null,
    treatments: []
  }, {
    injuryName: 'Blunt Force Trauma',
    subInjuries: null,
    treatments: [{
      label: 'C-Collar and Spine Board',
      reportText: 'Patient made stable with a c-collar and a spine board.',
      emote: ''
    },{
      label: 'IV saline administered.',
      reportText: 'IV saline administered.',
      emote: ''
    },{
      label: 'Ice Packs',
      reportText: 'Ice packs applied to bruising.',
      emote: ''
    },]
  }, {
    injuryName: 'First Degree Burn',
    subInjuries: null,
    treatments: [{
      label: 'Cool With Saline',
      reportText: 'Patient\'s burns cooled with saline.',
      emote: ''
    },{
      label: 'Aloe Gel',
      reportText: 'Aloe gel applied.',
      emote: ''
    },{
      label: 'Advise Patient',
      reportText: 'Patient advised to stay out of the sun and to keep burn site away from sources of elevated heat.',
      emote: ''
    }]
  }, {
    injuryName: 'Second Degree Burn',
    subInjuries: null,
    treatments: [{
      label: 'Cool With Saline',
      reportText: 'Patient\'s burns cooled with saline.',
      emote: ''
    },{
      label: 'Aloe Gel',
      reportText: 'Aloe gel applied.',
      emote: ''
    },{
      label: 'Burn Dressing Applied',
      reportText: 'Burn dressing applied to wound.',
      emote: ''
    },{
      label: 'Advise Patient',
      reportText: 'Patient advised to reapply aloe and change bandage every 24 hours until wound has healed.',
      emote: ''
    }]
  }, {
    injuryName: 'Third Degree Burn',
    subInjuries: null,
    treatments: [{
      label: 'Oxygen Mask',
      reportText: 'Oxygen mask applied to patient.',
      emote: ''
    },{
      label: 'IV Saline',
      reportText: 'IV saline administered.',
      emote: ''
    },{
      label: 'Burn Dressings',
      reportText: 'All burns covered with burn dressings.',
      emote: ''
    },{
      label: 'Oxygen Monitor',
      reportText: 'Patient hooked up to oxygen monitor.',
      emote: ''
    },{
      label: 'C-Collar / Spineboard',
      reportText: 'Patient was made stable with a C-collar and spine board.',
      emote: ''
    },]
  }, {
    injuryName: 'Drowning',
    subInjuries: null,
    treatments: []
  }, {
    injuryName: 'Fall',
    subInjuries: null,
    treatments: [{
      label: '',
      reportText: '',
      emote: ''
    },{
      label: '',
      reportText: '',
      emote: ''
    },{
      label: '',
      reportText: '',
      emote: ''
    },{
      label: '',
      reportText: '',
      emote: ''
    },{
      label: '',
      reportText: '',
      emote: ''
    },]
  }, {
    injuryName: 'GSW',
    subInjuries: null,
    treatments: [{
      label: 'Oxygen Mask',
      reportText: 'Oxygen mask applied securely to the patient.',
      emote: ''
    },{
      label: 'Wounds packed',
      reportText: 'Patients wounds were packed with gauze to stop bleeding.',
      emote: ''
    },{
      label: 'IV Saline and TXA',
      reportText: 'IV saline and TXA administered.',
      emote: ''
    },{
      label: 'Flush/Pack/Wrap',
      reportText: 'Wounds flushed with IV saline, packed with fresh gauze, and wrapped in bandages.',
      emote: ''
    },{
      label: 'C-Collar',
      reportText: 'Patient was made stable with a C-collar and spine board.',
      emote: ''
    }]
  }, {
    injuryName: 'Head Wound',
    subInjuries: null,
    treatments: []
  }, {
    injuryName: 'Head Trauma',
    subInjuries: null,
    treatments: []
  }, {
    injuryName: 'Multiple GSW\'s',
    subInjuries: null,
    treatments: [{
      label: 'Oxygen Mask',
      reportText: 'Oxygen mask applied to patient.',
      emote: ''
    },{
      label: 'Wounds packed',
      reportText: 'Patients wounds were packed with gauze to stop bleeding.',
      emote: ''
    },{
      label: 'IV Saline and TXA',
      reportText: 'IV saline and TXA administered.',
      emote: ''
    },{
      label: 'Flush/Pack/Wrap',
      reportText: 'Wounds flushed with IV saline, packed with fresh gauze, and wrapped in bandages.',
      emote: ''
    }]
  }, {
    injuryName: 'MVA',
    subInjuries: null,
    treatments: [{
      label: 'C-Collar',
      reportText: 'C-Collar applied to securely to the patient.',
      emote: ''
    },{
      label: 'C-Collar and oxygen mask applied securely to the patient patient.',
      reportText: 'C-Collar and oxygen mask applied securely to the patient patient.',
      emote: ''
    },{
      label: 'IV Saline',
      reportText: 'IV saline started.',
      emote: ''
    },{
      label: 'Wounds Flushed',
      reportText: 'Open wounds flushed with saline, covered in gauze, and wrapped in bandages.',
      emote: ''
    },{
      label: 'Ice Packs',
      reportText: 'Ice packs applied to bruises.',
      emote: ''
    },{
      label: 'Spine Board',
      reportText: 'Patient secured with spine board.',
      emote: ''
    },]
  }, {
    injuryName: 'Stabbing',
    subInjuries: null,
    treatments: [{
      label: 'Oxygen Mask',
      reportText: 'Oxygen mask applied to patient.',
      emote: ''
    },{
      label: 'Wounds packed',
      reportText: 'Patients wounds were packed with gauze to stop bleeding.',
      emote: ''
    },{
      label: 'IV Saline and TXA',
      reportText: 'IV saline and TXA administered.',
      emote: ''
    },{
      label: 'Flush/Pack/Wrap',
      reportText: 'Wounds flushed with IV saline, packed with fresh gauze, and wrapped in bandages.',
      emote: ''
    },{
      label: 'C-Collar',
      reportText: 'Patient was made stable with a C-collar and spine board.',
      emote: ''
    }]
  }, {
    injuryName: 'Starvation/Dehydration/Low Blood Sugar',
    subInjuries: null,
    treatments: []
  }, {
    injuryName: 'Drug Overdose',
    subInjuries: null,
    treatments: []
  }, {
    injuryName: 'Other',
    subInjuries: null,
    treatments: []
  },
]

export const CallType = [ '911 Call/Text', '10-52', '10-13', '10-13B', '10-78', 'Other']

export const BurnDegrees = ['First Degree', 'Second Degree', 'Third Degree', 'Forth Degree']

export const AirwayOptions = ['Clear', 'Partially Obstructed', 'Fully Obstructed']

export const BreathingTypes = ['Normal', 'Labored', 'Not Breathing']

export const SkinConditions = ['Normal', 'Minor Bruising', 'Moderate Bruising', 'Severe Bruising']

export const CirculationDescriptions = ['Normal', 'Minor bleeding', 'Moderate bleeding', 'Severe bleeding', 'Extreme bleeding']

export const PulseDescription = [
  'Mild Bradycardia (<60 BPM)',
  'Normal (60-100 BPM)',
  'Mild Tachycardia (100-120 BPM)',
  'Moderate Tachycardia (121-170 BPM)',
  'Severe Tachycardia (180+ BPM)',
  'No Pulse',
  'Custom'
]

export const AlernessOptions = [
  'Patient was responsive, alert, and oriented on scene.',
  'Patient was responsive/alert, but not oriented on scene.',
  'Patient was non-responsive/unconscious on scene.'
]

export const SafrTreatmentDischargeOptions = [
  'Patient was transported to hospital by a member of SAFR.',
  'Patient was transported to hospital by a member of Law Enforcement.',
  'Patients wounds did not warrant transport to a hospital, and they were discharged on-scene.',
  'Patient refused transportation to the hospital. Patient signed an AMA Waiver and was released on scene.'
]

export const HospitalStaffOptions = ['Local Nurses', 'Specific Staff Member']

export const HospitalLocations = ['Mt. Zonah Medical Center', 'Sandy Medical Center', 'Paleto Medical Center']

export function GetPainDescription(indicatedPainLevel: number) {
  switch (true) {
    case indicatedPainLevel >= 1 || indicatedPainLevel <= 3:
      return 'Mild Pain'
    case indicatedPainLevel >= 4 || indicatedPainLevel <= 6:
      return 'Moderate Pain'
    case indicatedPainLevel >= 7 || indicatedPainLevel <= 10:
      return 'Severe Pain'
    case indicatedPainLevel == 0:
    default:
      return 'No Pain'
  }
}

export function GetHighestPainMedication(indicatedPainLevel: number) {
  switch (true) {
    case indicatedPainLevel >= 1 || indicatedPainLevel <= 3:
      return 'Ibuprofen/Tylenol'
    case indicatedPainLevel >= 4 || indicatedPainLevel <= 6:
      return 'Tramadol'
    case indicatedPainLevel >= 7 || indicatedPainLevel <= 10:
      return 'Morphine'
    case indicatedPainLevel == 0:
    default:
      return 'No Pain'
  }
}

export const PainMedicationAdministrationOptions = [
  'Patient was given pain medication',
  'Patient declined pain medication',
  'Patient did not qualify for pain medication'
]

export const PainMedicationOptions = ['Ibuprofen', 'Tylenol', 'Tramadol', 'Morphine', 'Narcan']
